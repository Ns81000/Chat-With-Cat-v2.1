...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // Clear the response cache on extension reload/update
  responseCache.clear();
  console.log('Cache cleared on extension installation/update');
...
function sendResponseToContent(responseText, tabId, retries = 3, delay = 500) {
  ...
  chrome.tabs.get(tabId, (tab) => {
    if (chrome.runtime.lastError) {
      console.error('Tab no longer exists or cannot be accessed:', chrome.runtime.lastError.message);
      return;
    }
    ...
    chrome.tabs.sendMessage(tabId, {
      ...
      if (chrome.runtime.lastError) {
        console.warn('Error sending response to content script:', chrome.runtime.lastError.message);
...
const fetchAIResponse = debounce(async (text, tabId) => {
  try {
    ...
    console.log(`Fetching AI response for text: "${text.substring(0, 50)}..."`);
    console.log(`Using provider: ${activeProvider}, model: ${config.selectedModel}`);
...
// Add a dedicated message listener for cache management and ping handling
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  // Handle ping request to check if background service is ready
  ...
  if (message.action === 'clearResponseCache') {
    try {
      const cacheSize = responseCache.cache.size;
      responseCache.clear();
      console.log(`Background cache cleared: Removed ${cacheSize} items`);
      sendResponse({ success: true, itemsCleared: cacheSize });
    } catch (error) {
      console.error('Error clearing background cache:', error);
      sendResponse({ success: false, error: error.message });
    }
    return true;
  }
  
  // Handle other messages from content script
...
async function processQuestionWithLLM(provider, prompt, config) {
  try {
    ...
    const cacheKey = `question_${normalizedPrompt}_${provider}_${config.selectedModel}_${Date.now()}`;
    ...
    // Process the question with retry logic - added longer timeout for API calls
    console.log('Processing question with provider:', provider);
    
    // Wrap in a timeout promise
    const responsePromise = retryWithBackoff(
    ...
    console.log('Received response from provider:', response ? response.substring(0, 50) + '...' : 'No response');
