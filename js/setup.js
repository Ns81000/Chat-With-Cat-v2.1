...
document.addEventListener('DOMContentLoaded', function() {
  ...
  providerSelect.addEventListener('change', () => {
    ...
    document.querySelectorAll('.provider-setup').forEach(el => el.style.display = 'none');
    ...
    chrome.storage.local.get(['apiConfig'], function(data) {
      ...
      chrome.storage.local.set({ apiConfig, activeProvider: provider }, function() {
        if (chrome.runtime.lastError) {
          showStatus('Failed to save configuration: ' + chrome.runtime.lastError.message, 'error');
        } else {
